...
require_once 'src/jpgraph_bar.php';     //导入Jpgraph类库的柱状图功能
	include('conn.php');
//--------------------------查出X轴名称
	$sqlkm = 'SELECT DISTINCT kmmc FROM kemu;';
	$rkm = mysql_query($sqlkm);
	while($relkm = mysql_fetch_array($rkm)){
		$datas[] = $relkm[0];
	}
//--------------------------查出数值
	$sqlavg = 'SELECT AVG(cj) FROM cj GROUP BY kmid';
...
$barplot->value->Show();
$graph->title->Set(iconv("utf-8","gb2312","所有学生科目平均成绩"));     //设置统计图标题
$graph->xaxis->title->Set(iconv("utf-8","gb2312","科目"));     //设置X轴名称
$graph->xaxis->SetTickLabels($datas);
$graph->yaxis->title->Set(iconv("utf-8","gb2312",'平均成绩'));     //设置y轴名称
